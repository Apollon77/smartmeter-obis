...
function ObisMeasurement(medium, channel, measurement, measureType, tariffRate, previousMeasurement) {
    ...
    if (typeof medium === 'string' && typeof channel === 'undefined') {
        var obisArr = medium.match(/^([0-9]{1,3})-([0-9]{1,3}):([0-9CFLP]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})\*([0-9]{1,3})$/);
        ...
        if (obisArr && obisArr.length > 1) {
            ...
            medium = new Buffer(medium,'hex');
        }
        else {
            obisArr = medium.match(/^([0-9CFLP]{1,3})\.?([0-9]{0,3})\.?([0-9]{0,3})\*?([0-9]{0,3})$/);
            if (obisArr && obisArr.length > 1) {
...
ObisMeasurement.prototype.idToString = function idToString() {
    var obisStr = "";
    if (this.medium !== undefined && this.channel !== undefined) obisStr += this.medium + "-" + this.channel;
    if (this.measurement !== undefined) {
        ...
        if (this.previousMeasurement !== undefined) obisStr += "*" + this.previousMeasurement;
    }
    return obisStr;
};
