    ...
    if (crcCheck) {
        for (var msg in smlFile.messages) {
            if (smlFile.messages[msg].getMessageTag() === OpenSml.Constants.GET_LIST_RESPONSE) {
                ...
                if (smlFile.messages[msg].isValid() || this.options.protocolSmlIgnoreInvalidCRC) Object.assign(result, this.prepareResult(smlFile.messages[msg].getMessageBody().getValList()));
            }
            else if (smlFile.messages[msg].getMessageTag() === OpenSml.Constants.GET_PROFILE_LIST_RESPONSE) {
                crcCheck &= smlFile.messages[msg].isValid();
                if (smlFile.messages[msg].isValid() || this.options.protocolSmlIgnoreInvalidCRC) Object.assign(result, this.prepareResult(smlFile.messages[msg].getMessageBody().getPeriodList()));
            }
    	}
    }
