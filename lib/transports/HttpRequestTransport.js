...
HttpRequestTransport.prototype.process = function process() {
    ...
    var timeoutId = setTimeout(() => controller.abort(), this.options.transportHttpRequestTimeout);
    
    fetch(this.options.transportHttpRequestUrl, {
        method: 'GET',
        ...
        if (error.name === 'AbortError') {
            errorMessage = 'Request timeout after ' + self.options.transportHttpRequestTimeout + 'ms';
        } else {
            errorMessage = error.message;
        }
        
        self.options.logger('ERROR ON HTTP REQUEST: ' + errorMessage);
