...
SerialResponseTransport.prototype.scheduleNextRun = function scheduleNextRun() {
    if (!this.stopRequests) {
        ...
        if (this.options.requestInterval > 0) {
            ...
            this.currentData = null;
            this.currentDataOffset = 0;
            if (this.serialComm && this.serialConnected) this.serialComm.close(function(err) {
                if (err) {
        ...
        if (this.serialComm) this.serialComm.close(function(err) {
            if (err) {
                this.options.logger('ERROR CLOSING SERIALPORT');
            }
        });
    }
};
