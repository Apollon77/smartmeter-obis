...
function SerialRequestResponseTransport(options, protocol) {
    ...
    if (!this.options.transportSerialMaxBufferSize) this.options.transportSerialMaxBufferSize = 300.000;
}
...
SerialRequestResponseTransport.prototype.init = function init() {
    ...
    if (!this.serialComm) {
        ...
        if (this.options.debug === 2) this.options.logger('OPEN SERIALPORT: ' + this.options.transportSerialBaudrate + ' '  + this.options.transportSerialDataBits + ' '  + this.options.transportSerialStopBits + ' '  + this.options.transportSerialParity);
